...
func New(log *slog.Logger, createCategoryHandler CreateCategoryHandler) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if errors.Is(err, io.EOF) {
			log.Error("empty request body", sl.Error(err), slog.String("op", op))
		...
		if err != nil {
			log.Error("failed to decode request", sl.Error(err), slog.String("op", op))
			w.WriteHeader(http.StatusBadRequest)
		...
		if err := validator.New().Struct(req); err != nil {
			...
			log.Error("validation failed", sl.Error(err), slog.String("op", op))
			w.WriteHeader(http.StatusBadRequest)
		...
		if err != nil {
			log.Error("failed to create category", sl.Error(err), slog.String("op", op))
